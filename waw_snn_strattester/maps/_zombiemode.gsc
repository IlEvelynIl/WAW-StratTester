...
init_dvars()
{
	...
	maps\_strattester::init_st_dvars();
}
	...
	for( ;; )
	{
		...
		if( isdefined( self.initialized ) )
		{
			if( self.initialized == false )
			{
				...
				self thread maps\_strattester::on_player_spawn();
			}
		}
	}
}
...
#/
		...
		// calc from strat tester script
		maps\_strattester::calculate_zombie_speed();
		maps\_strattester::calculate_zombie_health();
...
{
	//players = getplayers();
	//players[0] SetPerk( "specialty_armorvest" );
	//players[0].maxhealth = 160;
	//players[0].health = 160;
	...
	while(true)
	{
		...
		{
			// PI_CHANGE_BEGIN
			// JMA - making sure that the player isn't in magic bullet shield so that it doesn't crash
